...
export default function Index() {
  ...
  const fundingGap = crisisZones.reduce((s, z) => s + (z.needed_usd - z.funded_usd), 0);
  ...
  const handleDonate = useCallback((type: 'zakat' | 'sadaqah', amount: number) => {
    ...
    setSelectedZone(null); // close drawer

    // Fly out to world view then trigger beam
    setTimeout(() => {
  ...
  const handleBeamComplete = useCallback(() => {
    setBeamTarget(null);
    toast(`✓ ${formatCAD(donatingAmount)} sent to ${donatingZoneName} — you'll receive verified impact updates`, {
      duration: 4000,
      style: {
  ...
  return (
    <div className="w-screen h-screen overflow-hidden" style={{ backgroundColor: '#060810' }}>
      <TopBar
